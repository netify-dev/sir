...
cluster-robust uncertainty by default for supported static fits, both
directed and symmetric (undirected). For dynamic (4D) `W` the default
accessors fall back to classical Wald inference (an explicit
`type = "cluster"` still errors); use `boot_sir(fit, type = "dyad")` for
dynamic `W` and for full-bilinear bipartite fits. Request classical
intervals anytime with `se.type = "classical"`.
